...
# 视频文件路径
video_path = "DB/output_video.mp4"  # 替换为文件夹中视频的路径

# 使用视频文件作为输入源
cap = cv2.VideoCapture(video_path)

# 设置视频帧宽度和高度（根据需要可设置，也可能无法生效，取决于视频源）
...
# 主循环
frame_delay = 400 # 帧延迟时间（毫秒），可以根据需要调整
while True:
    ...
    if len(faceCurFrame) == 0:
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            matches = face_recognition.compare_faces(facesEncodingList, encodeFace, tolerance=0.57)
